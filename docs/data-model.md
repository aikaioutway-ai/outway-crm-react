...
## Пеня — начисляется отдельной Edge Function, не клиентским кодом

`utils/pricing.ts` содержит функцию `calcPenalty()` (0 до 5-го числа, потом +100/день, потолок 15% от суммы), но **она нигде не вызывается в `src/`** — ни в одном компоненте или сервисе. Фактическая пеня начисляется на сервере: `supabase/functions/daily-penalty/index.ts`, по cron `0 8 7-31 * *` (каждый день с 7 по 31 число, в 08:00), на таблице `v2_charges`. Логика идентична описанной в CLAUDE.md (100 сом/день, потолок 15%, пеня не растёт, если уже занята "На проверке" через `is_frozen`), но реализация — не `pricing.ts`.

**ТРЕБУЕТ ПОДТВЕРЖДЕНИЯ:** `daily-penalty` читает и пишет колонки `penalty_amount`, `is_frozen`, `penalty_last_charged_on` на таблице `v2_charges`. В определении `v2_charges` внутри `crm_v2_schema.sql` этих колонок нет — они есть только в определении старой (неиспользуемой) таблицы `charges` из `finance_schema.sql`. Файл `supabase/v2_charges_penalty_idempotency.sql` добавляет только `penalty_last_charged_on` и явно помечен как "запустить вручную в Supabase SQL Editor" — то есть не через отслеживаемую миграцию. Колонки `penalty_amount`/`is_frozen` на `v2_charges`, скорее всего, тоже были добавлены вручную и не отражены ни в одном файле схемы в репозитории. Нельзя подтвердить их точное определение (тип, default) без доступа к живой базе — фиксирую как факт использования в коде, но не факт схемы.
