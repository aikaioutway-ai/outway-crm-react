...
export function useB2BPaymentsQuery(): UseQueryResult<B2BPaymentRecord[]> {
  return useQuery<B2BPaymentRecord[]>({ queryKey: B2B_QUERY_KEYS.payments, queryFn: fetchB2BPayments });
}

export default function useB2BPayments(): B2BPaymentRecord[] {
  const { data = [] } = useB2BPaymentsQuery();
  return data;
}
