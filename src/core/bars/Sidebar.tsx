...
import { Users, Wallet, Route, Receipt, UserCog, ChevronLeft, ChevronRight, LogOut, SlidersHorizontal, Columns3, ClipboardList } from 'lucide-react';
import { UserRole } from '../../types';

export type NavSection = 'families' | 'employees' | 'cashier' | 'logistics' | 'drivers' | 'payroll' | 'expenses' | 'timesheet' | 'settings' | 'bank_statement';
...
export function getAllowedSections(role: UserRole): NavSection[] {
  if (role === 'admin')        return ['families', 'employees', 'cashier', 'logistics', 'expenses', 'timesheet'];
  if (role === 'gen_director') return ['families', 'employees', 'cashier', 'logistics', 'expenses', 'timesheet'];
  if (role === 'director')     return ['families', 'cashier', 'logistics', 'timesheet'];
  if (role === 'manager')      return ['families'];
  if (role === 'logist')        return ['logistics'];
  if (role === 'senior_logist') return ['logistics', 'timesheet'];
