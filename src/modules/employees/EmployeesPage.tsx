...
function restrictedRoleFor(viewerRole: UserRole): EmployeeRole | null {
  ...
  if (viewerRole === 'senior_logist') return 'logist';
  return null;
}
...
export default function EmployeesPage({ viewerRole }: { viewerRole: UserRole }) {
  ...
  useEffect(() => { void load(); }, []);
  const scoped = useMemo(() => restrictedRole ? rows.filter(row => row.role === restrictedRole) : rows, [rows, restrictedRole]);
...
function EmployeeCardModal({ employee, restrictedRole, onClose, onChanged, onDeleted }: { employee: Employee | null; restrictedRole: EmployeeRole | null; onClose: () => void; onChanged: (employee: Employee) => void; onDeleted: () => void }) {
  const [draft, setDraft] = useState<EmployeeDraft>(employee ? toDraft(employee) : { ...EMPTY_DRAFT, role: restrictedRole ?? EMPTY_DRAFT.role, position: restrictedRole ? roleLabel(restrictedRole) : EMPTY_DRAFT.position, startDate: today() });
