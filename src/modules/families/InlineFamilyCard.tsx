...
export default function InlineFamilyCard({ family, onClose, userRole = 'manager', userName = 'Менеджер' }: Props) {
  ...
  async function addAudit(action: string, field: string, oldVal: string, newVal: string) {
    try {
      await addV2Audit({ actorName: userName, action, entityType: field, entityId: family.id, oldValue: oldVal, newValue: newVal });
    } catch { /* таблица ещё не создана */ }
  }
  ...
  async function handleSavePayment(payment: FamilyPayment, updates: Partial<FamilyPayment>): Promise<boolean> {
    try {
      await updateFamilyPayment(payment.id, {
        amount: updates.amount, paymentType: updates.paymentType,
  ...
  return (
    <div style={{ background: '#FAFBFF', borderTop: '1px solid #E8EAF6' }}>

      {/* ─── HEADER ─── */}
      <div style={{ background: '#312E81', padding: '12px 20px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#fff', letterSpacing: -0.3, lineHeight: 1.15 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 0 }}>
          <Chip label="НАЧИСЛЕНО" value={money(totalCharged)} />
          <Chip label="ПЛАТЕЖИ" value={money(totalPaid)} chipBg="#D1FAE5" chipColor="#065F46" />
          <Chip
          ...
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              {t.key === 'history' && audit.length > 0 && (
                <span style={{ background: 'rgba(255,255,255,0.25)', color: '#fff', borderRadius: 10, fontSize: 9, padding: '1px 5px', fontWeight: 700 }}>
                  {audit.length}
                </span>
              )}
            </button>
      ...
      {/* ─── CONTENT ─── */}
      <div style={{ padding: '14px 20px', maxHeight: 520, overflowY: 'auto' }}>
...
function Chip({ label, value, sub, chipBg, chipColor }: {
  label: string; value: string; sub?: string; chipBg?: string; chipColor?: string;
}) {
  return (
    <div style={{ background: chipBg ?? 'rgba(255,255,255,0.13)', borderRadius: 8, padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      {sub && <div style={{ fontSize: 10, fontWeight: 500, color: chipColor ? chipColor + '99' : 'rgba(255,255,255,0.5)' }}>{sub}</div>}
    </div>
  );
}
