...
export default function B2BCashier({ onOpenOrder }: B2BCashierProps) {
  ...
  if (detail === 'Поступило за месяц' || detail === 'На проверке' || detail === 'Расчётный налог B2B') {
    ...
    detailRows = expenses.filter(row => row.source === 'manual' && inPeriod(row.expenseDate)).map(row => ({ id: row.id, orderId: orders.find(order => order.number === row.orderNumber)?.id, order: row.orderNumber || 'Без заказа', description: [row.purpose, row.comment, formatB2BPaymentMethod(row.method)].filter(Boolean).join(' · '), date: row.expenseDate, amount: row.netAmount }));
    ...
    detailRows = [
      ...
      ...payouts.map(row => ({ id: `d-${row.id}`, orderId: row.orderId, order: orders.find(order => order.id === row.orderId)?.number || '', description: row.driverName, date: row.paymentDate, amount: -row.netAmount })),
      ...expenses.filter(row => row.source === 'manual').map(row => ({ id: `e-${row.id}`, orderId: orders.find(order => order.number === row.orderNumber)?.id, order: row.orderNumber || 'Без заказа', description: row.purpose, date: row.expenseDate, amount: -row.netAmount })),
    ];
  } else if (detail) {
  ...
  return <section className="b2b-cashier">
    ...
    <div className="b2b-cashier-workspace">
      ...
      <section className="b2b-cashier-panel">
        ...
        <div className="b2b-cashier-table-wrap"><table><thead><tr><th>Заказ</th><th>Водитель</th><th>Дата поездки</th><th className="number">Начислено</th><th className="number">Оплачено</th><th className="number">Получено водителем</th><th className="number">Остаток</th></tr></thead><tbody>{summary.driverPayables.length ? summary.driverPayables.map(row => <tr key={row.orderId} onClick={() => openOrder(row.orderId)}><td><button type="button" className="b2b-cashier-order-link" onClick={() => onOpenOrder?.(row.orderId)}>{row.orderNumber}</button></td><td>{row.driverName}</td><td>{displayDate(row.tripDate)}</td><td className="number">{money(row.accrued)}</td><td className="number">{money(row.paidGross)}</td><td className="number">{money(row.paidNet)}</td><td className="number debt">{money(row.remaining)}</td></tr>) : <tr><td colSpan={7} className="empty">Неоплаченных обязательств нет</td></tr>}</tbody></table></div>
      </section>
    </div>
