...
export default function B2BLogistics({ canPay = true, onOpenOrder }: { canPay?: boolean; onOpenOrder?: (id: string) => void }) {
  ...
  const openOrderCard = (order: B2BOrder) => { if (onOpenOrder) { onOpenOrder(order.id); return; } setOrderCardTab('main'); setSelectedOrder(order); };
  ...
  const saveAssignment = async (event: FormEvent) => {
    ...
    try {
      const assignment = await saveB2BAssignment(assignmentOrder, driver.driverId, price);
      updateOrder(assignmentOrder.id, { assignmentId: assignment.id, driverId: driver.driverId, driverName: driver.fullName, driverPricePerUnit: assignment.driverPricePerUnit, driverTotal: assignment.driverTotal, status: 'driver_assigned' });
  ...
  const openPayment = (order: B2BOrder) => {
    if (!canPay) return;
    setPaymentOrder(order); setPaymentAmount(''); setPaymentMethod('cash'); setPaymentOrderNumber('');
  ...
  const savePayment = async (event: FormEvent) => {
    ...
    if (requiresB2BPaymentOrder(paymentMethod) && !paymentOrderNumber.trim()) return setPaymentError('Укажите номер платёжного поручения.');
    try {
      await createB2BDriverPayout(paymentOrder, amount, paymentMethod, paymentDate, paymentComment.trim(), paymentOrderNumber.trim() || undefined);
  ...
  const actionButtons = (order: B2BOrder) => {
    ...
    return <div className="b2b-logistics-row-actions" onClick={event => event.stopPropagation()}>
      ...
      {canPay && !!order.driverName && remaining > 0 && <button type="button" title="Выплата водителю" onClick={() => openPayment(order)}><CreditCard size={14} /></button>}
  ...
  return (
    <section className="b2b-logistics">
      ...
      {canPay && paymentOrder && <div className="b2b-modal-overlay" onMouseDown={event => { if (event.target === event.currentTarget) setPaymentOrder(null); }}><div className="b2b-logistics-dialog" role="dialog" aria-modal="true"><header><div><h2>Выплата водителю</h2><p>{paymentOrder.number} · {paymentOrder.driverName}</p></div><button type="button" onClick={() => setPaymentOrder(null)}><X size={18} /></button></header><form onSubmit={savePayment}><div className="b2b-logistics-payment-summary"><span>Начислено <b>{totalFor(paymentOrder).toLocaleString()} сом</b></span><span>Выплачено <b>{paidFor(paymentOrder.id).toLocaleString()} сом</b></span><span>Остаток <b>{remainingFor(paymentOrder).toLocaleString()} сом</b></span></div><label><span>Сумма, сом *</span><input autoFocus type="number" min="1" max={remainingFor(paymentOrder)} value={paymentAmount} onChange={event => setPaymentAmount(event.target.value)} /></label><label><span>Способ оплаты</span><select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value as B2BPaymentMethod)}>{B2B_PAYMENT_METHODS.map(method => <option key={method.value} value={method.value}>{method.label}</option>)}</select></label><label><span>№ платёжного поручения{requiresB2BPaymentOrder(paymentMethod) ? ' *' : ''}</span><input required={requiresB2BPaymentOrder(paymentMethod)} value={paymentOrderNumber} onChange={event => setPaymentOrderNumber(event.target.value)} placeholder={requiresB2BPaymentOrder(paymentMethod) ? 'Обязательно' : 'Для наличных необязательно'} /></label><label><span>Дата *</span><input type="date" value={paymentDate} onChange={event => setPaymentDate(event.target.value)} /></label><label className="full"><span>Комментарий</span><input value={paymentComment} onChange={event => setPaymentComment(event.target.value)} /></label>{paymentError && <div className="b2b-form-error">{paymentError}</div>}<div className="b2b-form-actions"><button type="button" className="b2b-cancel-button" onClick={() => setPaymentOrder(null)}>Отмена</button><button type="submit" className="b2b-primary-button">Сохранить выплату</button></div></form></div></div>}
      {selectedOrder && (
