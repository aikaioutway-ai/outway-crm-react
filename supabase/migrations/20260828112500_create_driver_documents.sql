-- Документы водителей и хранилище сканов.
...
create trigger trg_v2_driver_documents_updated_at
before update on public.v2_driver_documents
for each row execute function public.v2_touch_updated_at();

create index if not exists idx_v2_driver_documents_driver
  on public.v2_driver_documents(driver_id);
...
begin
  if not exists (
    ...
    create policy "v2 authenticated read" on public.v2_driver_documents
  ...
  if not exists (
    ...
    create policy "v2 authenticated write" on public.v2_driver_documents
  ...
  if not exists (
    ...
    create policy "v2 anon read" on public.v2_driver_documents
  ...
  if not exists (
    ...
    create policy "v2 anon write" on public.v2_driver_documents
